...
var ui =
{
	...
	initialise_last: function()
	{
		document.getElementById("d_button_record").addEventListener('click',
			function(event)
			{
				media.button_event(key.key_2_button(''));
			}.bind(this)
		);
	...
	{
		document.getElementById("d_indicator_recording").className = (true === active) ? "indicator_recording_active" : "indicator_recording_inactive";
	},
	indicator_record_buffer_set: function(active)
	{
		document.getElementById("d_indicator_record_buffer").className = (true === active) ? "indicator_record_buffer_active" : "indicator_record_buffer_inactive";
	},
	indicator_playing_set: function(active)
	{
		document.getElementById("d_indicator_playing").className = (true === active) ? "indicator_playing_active" : "indicator_playing_inactive";
	},
	indicator_play_buffer_set: function(active)
	{
		document.getElementById("d_indicator_play_buffer").className = (true === active) ? "indicator_play_buffer_active" : "indicator_play_buffer_inactive";
	},
};
